...
const typeDef =`
  extend type Query {
    ...
    findMoviesByDirectorId(directorId: String): [Movie]!
  }
...
const resolvers = {
  ...
  Query: {
    ...
    findMoviesByDirectorId: async (root, args) => {
      if (!args.directorId) return [] 
      const directorMoviesPopulated = await Movie.find({ director: args.directorId }).populate('director')
